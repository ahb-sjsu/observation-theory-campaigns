...
CACHE = os.path.join(HERE, f"ieip_{os.environ.get('IEIP_TAG', 'v2')}_states.npz")

CONS_CACHE = os.path.join(HERE, f"ieip_{os.environ.get('IEIP_TAG', 'v2')}_consumer.npz")

TAG = os.environ.get("IEIP_TAG", "v2")

N_TEXT = int(os.environ.get("IEIP_N", "2600"))   # before dedupe/identical filtering

LAYERS = [int(x) for x in os.environ.get("IEIP_LAYERS", "6,12,18,24").split(",")]
...
MT_FWD = os.environ.get("IEIP_MT_FWD", "Helsinki-NLP/opus-mt-en-es")

MT_BWD = os.environ.get("IEIP_MT_BWD", "Helsinki-NLP/opus-mt-es-en")

BATCH = int(os.environ.get("IEIP_BATCH", "16"))
...
def main():
    ...
    json.dump(results, open(os.path.join(HERE, f"cr_ieip_{TAG}_result.json"), "w"), indent=2)

    print(f"wrote {HERE}/cr_ieip_{TAG}_result.json")
