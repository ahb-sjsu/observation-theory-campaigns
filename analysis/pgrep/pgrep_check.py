"""XPROTO-PG graded runner — bars per PREREG-XPROTO-PG.md. Reuses the
family's measurement (fam_pgrep.run_cell) unchanged; runs on seeds
...
(2026-08-19) -- the cooling-off is a code guard.

    python3 pgrep_check.py                  # graded run (needs seal)
    python3 pgrep_check.py --check-family   # pre-seal record check only

The pre-seal record check (the OT-18 practice): evaluate every bar
against the committed family record at enforcement granularity BEFORE
sealing. It needs no seal and moves no data.
"""
...
# fam_pgrep (and its psycopg dependency) is imported only on the graded
# path; --check-family is a pure record read.

PREREG = os.path.join(HERE, "PREREG-XPROTO-PG.md")
FAMILY_CONSTRUCTED = date(2026, 8, 19)
GRADED_SEEDS = [20260820, 20260821, 20260822]
...
def require_seal() -> date:
    ...
    if "STATUS: UNSEALED" in t or "STATUS: SEALED" not in t:
        sys.exit("REFUSED: PREREG-XPROTO-PG.md is not SEALED.")
...
def main():
    ...
    ap.add_argument("--family-record",
                    default=os.path.join(HERE, "PGREP-family.json"))
    args = ap.parse_args()
    ...
    import fam_pgrep as fam  # psycopg needed only here
    print(f"XPROTO-PG graded — sealed {seal_date}, seeds {GRADED_SEEDS}",
          flush=True)
    cells = []
    for seed in GRADED_SEEDS:
        print(f"cell seed {seed}…", flush=True)
        cells.append(fam.run_cell(seed))
    out = report(cells, "XPROTO-PG")
    out.update({"cell": "XPROTO-PG", "sealed": str(seal_date),
                "seeds": GRADED_SEEDS,
                "constants_from": "fam_pgrep.py (sealed grid)"})
    path = os.path.join(HERE, "XPROTO-PG-graded.json")
