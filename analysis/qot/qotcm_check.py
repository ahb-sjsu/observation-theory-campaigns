"""XPROTO-QOT-CM graded checker -- bars per PREREG-XPROTO-QOT-CM.md.
...
cost-matched sign death is FAIL and is reported, never discarded.
    ...
    python3 qotcm_check.py                     # sealed grading
    python3 qotcm_check.py --smoke-record QOTCMREP-shakedown.json
...
PREREG = os.path.join(HERE, "PREREG-XPROTO-QOT-CM.md")
GRADED_RECORD = os.path.join(HERE, "QOTCMREP-graded-raw.json")
OUT_RECORD = os.path.join(HERE, "XPROTO-QOT-CM-graded.json")

# ---- constants restated from the prereg; the line citation is the authority ----
FAMILY_CONSTRUCTED = date(2026, 8, 28)      # prereg line 3
GRADED_SEEDS = [20260906, 20260907, 20260908]   # prereg lines 4-5, 58
SHAKEDOWN_SEEDS = [0, 1, 2]                 # prereg lines 5, 88
# The fleet is shared with F-QOT-GRAD, so that cell's calibration seeds are
# forbidden here too (PREREG-XPROTO-QOT-GRAD.md line 50).
SIBLING_CAL_SEEDS = [990, 991, 992, 993, 994, 995, 996, 997]
MODE = "gnpy-coronet"                       # prereg line 6

LAMBDA_SHIFT_TOL = 0.10                     # prereg line 62 (B2)
D_RMS_FLOOR_DB = 0.1                        # prereg line 73 (MC2)
COST_RTOL = 1e-9                            # prereg line 74 (MC3), line 56
S_LO, S_HI = 0.25, 4.0                      # prereg line 55 (solver bracket)
N_BISECT = 200                              # prereg line 55
...
def require_seal() -> date:
    ...
    if "STATUS: UNSEALED" in t or "STATUS: DRAFT" in t or "STATUS: SEALED" not in t:
        sys.exit("REFUSED: PREREG-XPROTO-QOT-CM.md is not SEALED.")
...
def solver_guard(rec):
    """The declared solver constants must be the ones that produced the record."""
    c = rec.get("constants", {})
    return {
...
def grade_cell(c):
    ...
    return {
        # B1 (prereg line 60): signs survive cost matching and match the
        # nominal pair in the same run.
        "B1_signs_survive": bool(
            c["dR_R_cm"] < 0 < c["dR_P_cm"] and
            sign(c["dR_R_cm"]) == sign(c["dR_R_nom"]) != 0 and
            sign(c["dR_P_cm"]) == sign(c["dR_P_nom"]) != 0),
        # B2 (prereg line 62): lambda* stability under the declared tolerance.
...
def report(rec, label, smoke):
    ...
    for g in graded:
        ...
        print(f"  seed {g['seed']}: s={g['s_rebalance']} "
              f"cost_A={g['cost_A_bits']} cost_B_nom={g['cost_B_nominal_bits']} "
        ...
        print(f"          nom dR_R={g['dR_R_nom']} dR_P={g['dR_P_nom']} "
              f"lam*={g['lambda_star_nom']} | cm dR_R={g['dR_R_cm']} "
              f"dR_P={g['dR_P_cm']} lam*={g['lambda_star_cm']} "
...
def main():
    ...
    if args.smoke_record:
        ...
        print(f"XPROTO-QOT-CM checker smoke test on {os.path.basename(path)}",
              flush=True)
    ...
    if not os.path.exists(GRADED_RECORD):
        sys.exit("REFUSED: QOTCMREP-graded-raw.json missing -- run fam_qotcm.py "
    ...
    print(f"XPROTO-QOT-CM graded -- sealed {seal_date}, seeds {GRADED_SEEDS}",
          flush=True)
    out = report(rec, "XPROTO-QOT-CM", smoke=False)
    out.update({"cell": "XPROTO-QOT-CM", "sealed": str(seal_date),
