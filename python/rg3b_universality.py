#!/usr/bin/env python3
"""RG-3b the universality gate, corrected U4.

Protocol declared in RENORMALIZATION-TRACK.md before this run.
Identical to RG-3 in U1 through U3. U4 is corrected, from an
exactly-in-family start the channel decides whether the family
survives, decimation preserves it exactly and majority departs,
while for the out-of-family model both channels' reductions are
recorded as measurements of the dominant one-dimensional flow.
RG-3's U4 assumed the channel's departure would outrun the
substrate's convergence. Verdict computed from the measured items.
...
def main() -> int:
    record: dict = {"schema": "rg3b-universality-v1",
                    "label": "exploratory"}
    items = {}
    ...
    # U4 corrected, channel decides family survival from an
    # in-family start; out-of-family reductions recorded
    n12 = model_probs(12, 0.5, 0.0, 0.0)
    dn_dec, _ = family_residual(6, decimate(12, n12))
    dn_maj, _ = family_residual(4, majority3(12, n12))
    ...
    items["U4_channel_relative"] = (dn_dec <= 1e-12
                                    and dn_maj >= 1e-3)
    ...
    record["measured"] = {
        ...
        "u4_in_family_start_dec_maj": [dn_dec, dn_maj],
        "u4_out_of_family_micro_dec_maj": [dm, dd, dj]}
