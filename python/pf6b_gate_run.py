#!/usr/bin/env python3
"""PF6-002 governed gate run. Sealed under PREREG-PF6-002.
...
with members bound from the committed member probe so the audits
run on worldlines that actually fold. PREREG-PF6-001 returned
vacuous on fold-free members. Every constant is from the sealed
document. The verdict is computed from the bars.
"""
...
# (gap, field, pu0) bound from results/pf6-member-probe.json
...
def main() -> int:
    record: dict = {"schema": "prereg-pf6-002-v1",
                    "registration_id": "PREREG-PF6-002",
    ...
    for idx, (p, e, pu0) in enumerate(MEMBERS):
        ...
        for t0 in SHIFTS:
            mv = integrate(p, e, t0=t0, n_steps=N_STEPS,
                           pu0=pu0)
        ...
        sc = integrate(p, e, alpha=ALPHA_REPARAM,
        ...
        cells[f"P{p}_E{e:.6f}_pu{pu0:g}"] = {
            "fold_count": int(n_base),
            "probe_folds": PROBE_FOLDS[idx],
            "fold_count_even": bool(n_base % 2 == 0),
    ...
    b6_ok = b6_ok and total_folds >= MIN_TOTAL_FOLDS
    ...
    record["measured"] = {
        ...
        "all_fold_counts_even": bool(all(
            v["fold_count_even"] for v in cells.values())),
    ...
    record["declared"] = {"members": MEMBERS,
