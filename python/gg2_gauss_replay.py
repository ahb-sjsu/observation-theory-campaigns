#!/usr/bin/env python3
"""GG-2 the Gauss-law replay (exploratory).

Protocol declared in GAUGE-TRACK.md before this run. The GG-0
substrate under the deformed weights exp(K sum plaq + eps sum
links). At epsilon zero the exact zeros replicate, at every
positive epsilon the broken constraint is visible at first order
in the link sector while the invariant sector responds only at
second order. Constraint structure is an isolated algebraic point.
...
def main() -> int:
    record: dict = {"schema": "gg2-gauss-replay-v1",
                    "label": "exploratory"}
    items = {}
