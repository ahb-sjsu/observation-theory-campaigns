#!/usr/bin/env python3
"""CR-4c the backdoor audit ceiling, entropy bound (exploratory).
...
def main() -> int:
    record: dict = {"schema": "cr4c-backdoor-ceiling-v1",
                    "label": "exploratory"}
    ...
    def cond_with_trapdoor(d):
        """The holder of the scalar walks the same law by the
        inverse map, scalar-multiplying the observed point by the
        inverse of the scalar. CR-4b multiplied an abscissa as a
        scalar here, the named error this run corrects."""
        e = pow(d, order - 2, order)
        c1 = Counter()
        c12 = Counter()
        for s in range(1, order):
            s2 = x_of(table[s % order]) % order
            a_idx = (s2 * d) % order
            w1 = x_of(table[a_idx]) & OUT_MASK
            back_idx = (a_idx * e) % order
            s_next = x_of(table[back_idx]) % order
    ...
    h_scalar = ent([tot], tot)
    info_bound = h_scalar
    items["B3a_trapdoor_entropy_is_zero"] = abs(h_scalar) <= 1e-15
    items["B3b_route_agreement"] = abs(cond_k - cond_k_trap) <= 1e-9
    ...
    findings = {
        "B5_audit_blind_to_trapdoor": bool(info_bound <= 1e-15),
        "B6_audit_sees_leak_but_not_trapdoor": bool(
            mi_leak - mi_clean >= 0.5 and info_bound <= 1e-15)}
    ...
    record["measured"] = {
        ...
        "trapdoor_entropy_bits": float(h_scalar),
        "trapdoor_information_bound_bits": float(info_bound),
