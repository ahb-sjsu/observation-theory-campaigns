#!/usr/bin/env python3
"""PF4-008b the pole-order discriminator, halved timestep.
...
PF4-008 answered the discriminator and missed one bar. Its
Gudermannian cells drifted 1.9e-10 against a conservation bar of
1e-10 set from a narrower force, so this run halves the timestep
and changes nothing else. Fourth-order integration should bring the
drift to about 1e-11.
...
L_GRID = [1.5, 2.5, 3.5]
KAPPA_GRID = [1.0, 1.4, 1.8, 2.2, 2.6, 3.2]
DT = 1e-4
SPAN = 20.0
SHIFT = 3.0
H_BAR = 1e-10
...
def tilt_and_force(t, l_s, profile):
    ...
    if profile == "gudermann":
        return ((2.0 / math.pi) * np.arcsin(np.tanh(x)),
                (2.0 / math.pi) / np.cosh(x))
...
def main() -> int:
    record: dict = {"schema": "pf4-008b-pole-order-v1",
                    "label": "exploratory"}
    cells = {}
    worst_h = 0.0
    any_nonfinite = False
    for profile in ("gudermann", "sech2"):
        for l_s in L_GRID:
            a = POLE_DISTANCE * l_s
    ...
    gud = [cells[f"gudermann_L{l}"]["slope_vs_kappa"]
           for l in L_GRID]
    sec = [cells[f"sech2_L{l}"]["slope_vs_kappa"] for l in L_GRID]
    ...
    items["P3_width_universal"] = (gud_spread <= UNIV_TOL
                                   and sec_spread <= UNIV_TOL)
    items["P4_signal_in_range"] = all(
        SIGNAL_LO < v < SIGNAL_HI
        for c in cells.values() for v in c["e_res"])

    h1 = abs(gud_mean - SECH2_REFERENCE) <= H1_TOL * abs(
        SECH2_REFERENCE)
    h2 = abs(gud_mean - LORENTZ_REFERENCE) <= H2_TOL * abs(
        LORENTZ_REFERENCE)
    findings = {
        "F1_distance_only": bool(h1),
        "F2_order_matters": bool(h2),
        "F3_neither": bool(not h1 and not h2)}
    ...
    record["measured"] = {
        ...
        "sech2_mean_slope_this_run": sec_mean,
        ...
        "ratio_gudermann_over_sech2": float(gud_mean / sec_mean),
    ...
    out = Path(__file__).resolve().parents[1] / "results" \
        / "pf4-008b-pole-order.json"
    out.write_text(json.dumps(record, indent=2, sort_keys=True),
                   encoding="utf-8")
    print("gudermann", gud, "mean", gud_mean, "spread", gud_spread)
    print("sech2", sec, "mean", sec_mean, "spread", sec_spread)
