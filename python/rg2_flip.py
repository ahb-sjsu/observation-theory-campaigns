#!/usr/bin/env python3
"""RG-2 the flip in renormalization form (exploratory).

Protocol declared in RENORMALIZATION-TRACK.md before this run. One
bit of coarse-graining budget on the RG-1 cylinder, all 32767
bipartitions enumerated. Fidelity-optimal maximizes the compressed
marginal's entropy, task-optimal predicts the sign of the distant
column magnetization, and a grouping theorem fixes the task
optimum so the exhaustive search and the theorem certify each
other. Verdict computed from the measured items.
...
def main() -> int:
    record: dict = {"schema": "rg2-flip-v1",
                    "label": "exploratory"}
    items = {}
