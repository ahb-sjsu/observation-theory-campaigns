#!/usr/bin/env python3
"""PF4-008 the pole-order discriminator (exploratory).
...
That makes the comparison a controlled one. The two profiles have
singularities at the same distance and of different order, so the
run separates what the distance fixes from what the order fixes,
and the two possible answers are declared here before it runs.
...
def main() -> int:
    record: dict = {"schema": "pf4-008-pole-order-v1",
                    "label": "exploratory"}
    cells = {}
    worst_h = 0.0
    any_nonfinite = False
