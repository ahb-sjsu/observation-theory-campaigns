#!/usr/bin/env python3
"""PF-7e the detrended ceiling statistic (exploratory).
...
def main() -> int:
    record: dict = {"schema": "pf7e-detrended-v1",
                    "label": "exploratory"}
    ...
    items = {
        "E1_detrend_reduces_classical":
            bool(det_stat < raw_stat),
        "E2_quantum_survives_detrending":
            bool(q_det_stat >= 0.30),
        "E3_decision_recorded": True}
    findings = {
        "E4_classical_consistent_with_noise_after_detrend":
            bool(p_det > ALPHA),
        "E5_trend_is_real":
    ...
    record["verdict"] = {"value": verdict,
                         ...
                         "note": "E4 and E5 carry the ceiling "
                                 "reading, either outcome a result"}
    record["declared"] = {"n_members": N_MEMBERS,
                          "n_null": N_NULL, "seed": SEED,
                          "alpha": ALPHA,
                          "detrend": "declared linear fit in "
                                     "delay, removed from the "
                                     "curve and from every null "
                                     "curve alike",
                          "source_records":
