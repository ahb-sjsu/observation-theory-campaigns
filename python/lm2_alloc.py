...
Seeds: pilotA = 20261110; pilotB = 20261112; governed = 20261115.
"""
...
def main():
    ...
    random_subset = {cid: sorted(rng.sample(range(D), F_FINE)) for cid in CONSUMERS}
    ...
    for cid, spec in CONSUMERS.items():
        ...
        print(f"[{cid}] probe+eval done; aligned={aligned} oracle={oracle} "
              f"anti={anti} random={random_subset[cid]}", flush=True)

    out = {"campaign": "LM-2 matched-budget precision allocation (on the LM1-003-licensed instrument)",
           "mode": mode,
    ...
    os.makedirs("/home/claude/lm2", exist_ok=True)
    path = f"/home/claude/lm2/{mode}_log.json"
