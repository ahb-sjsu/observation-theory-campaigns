...
| 🔵 RG | Renormalization | Is universality a property of the substrate or of the declared coarse-graining channel? | RG-0/RG-1/RG-1b measured (RG-1 honest FAIL on a cancellation bar): correlation length is consumer-selected, sector-exact; RG-2..4 designed |
| ⚫ QD | Quantum Darwinism | Is objectivity multi-consumer redundancy, and does it emerge or must it be declared? | QD-0/QD-1/QD-1b measured (QD-1 honest FAIL = the recency-record finding): redundancy is declared-or-absent; QD-2..3 designed |
...
| 🔴 FT | Fluctuation theorems | What happens to Jarzynski and Crooks for a consumer with a restricted channel? | FT-0/FT-1 measured: the Jarzynski defect lives strictly between full sight and total blindness, and is always the declared channel's exact arithmetic; FT-2..3 designed |
| 🟤 GG | Gauge redundancy | Is gauge symmetry anything but the fiber structure of the observer's projection? | GG-0/GG-1 measured: every observer's bookkeeping is a relabeled invariant loop selected by its tree; GG-2..3 designed |
