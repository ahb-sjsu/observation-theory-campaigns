...
| 🔵 **RG-3b** corrected comparison | `[demonstrated-in-model]` ✅ | from the exactly-in-family start the channel decides family survival: one decimation keeps the residual at 4e-17, one majority blocking raises it to 0.0136. Universality is real but channel-shaped: WHAT converges is convergence itself; WHICH family it converges through is the observer's channel |
...
| ⚫ **QD-3** multi-consumer agreement | `[demonstrated-in-model]` ✅ | two Helstrom-optimal consumers on disjoint fragments: marginals match the closed form to 1e-13, Born routes agree to 1e-15, agreement curve 0.510→0.537→0.636→0.875→1.000 along the record-strength ladder — objectivity between observers is record strength made mechanical; every disagreement is Born arithmetic, not noise |
