...
Thirty-three sealed documents in the ledger (two instrument freezes,
five generator documents, one sealed-unrun track document, and
twenty-five preregistrations), every one verifiable by blob hash
...
anywhere). Status: **LM-1 sealed, run once, and honestly FAILED
(5/7 gates)** — B's oracle alignment missed its bar by 0.0013 and the
flip-support disjointness gate failed because the consumer's
weak-but-real read components are magnitude-comparable to its spurious
sensitivities, so task-oracle-anchored gates break exactly where the
consumer is imperfect. The instrument held throughout; no claim is
made; the diagnosis and the designated successor (LM1-002:
consumer-anchored held-out predictive gates, task-oracle alignment
demoted to a diagnostic) are on the record in the track document.
Track document:
