...
|---|---|---|---|
| 🔀 PF | Projection folds | Can a fold in how hidden motion maps to observed time imitate particle pair creation, quantitatively? | PF-0..8 measured; the PF4-002 summit question is ANSWERED by PF4-005b, a deterministic family whose exponent is fixed by the field's complex singularity distance and is width-universal to 0.26 percent, so it is dynamical and not a measure tail (per-crossing transfer only, unsealed); BOTH mandatory gates cleared: PF5-002 PASS on 8068 audited pair events, PF6-002 PASS on probe-placed folding members (PF5-001 and PF6-001 recorded vacuous, errors named); PF-7 and PF-8 now ungated; PF-8 decay gate MEASURED, PASS on all four bars — same-species 1→2 decay excluded by parity on every audited level, the 1→3 zigzag imitable, and a declared blind consumer produces 63 apparent 1→2 decays with zero detector-model mismatches |
