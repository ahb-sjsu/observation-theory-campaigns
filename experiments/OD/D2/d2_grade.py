"""Grades a D2 results.json against the sealed bars of PREREG-D2.md.
Usage: python d2_grade.py results.json --tols tolerances.json [--out grade.json]
tolerances.json carries REF (the frozen law's held-out error at the pilot) and FRAC_X."""
...
def grade(res: dict, tols: dict) -> dict:
    law = res["law"]["law"]; comp = res["law"]["competitor_nominal"]; REF = float(tols["REF"]); FX = float(tols["FRAC_X"])
    out = {"tolerances": tols, "law": law, "competitor": comp, "groups": {}, "bars": {}, "gate": None}
    fail = False; all_pass = True

    def rows_of(groups):
        return [r for w in res["worlds"] if w["group"] in groups for r in w["rows"] if "budget_rel" not in r]

    per_world = {w["name"]: {"group": w["group"], "rmse_law": rmse(law, [r for r in w["rows"] if "budget_rel" not in r]),
                             "rmse_competitor": rmse(comp, [r for r in w["rows"] if "budget_rel" not in r])} for w in res["worlds"]}
    out["worlds"] = per_world
    for g in ("train", "heldout", "unseen", "real", "scale"):
        rs = rows_of({g})
        out["groups"][g] = {"n": len(rs), "rmse_law": rmse(law, rs) if rs else None, "rmse_competitor": rmse(comp, rs) if rs else None}
    # L1 fresh seeds
    e = rmse(law, rows_of({"train", "heldout"})); l1 = e <= 1.5 * REF
    out["bars"]["L1_fresh_seeds"] = {"rmse": e, "limit": 1.5 * REF, "holds": l1}
    # L2 unseen
    uns = {n: v for n, v in per_world.items() if v["group"] == "unseen"}
    pooled_u = out["groups"]["unseen"]["rmse_law"]
    ran = pooled_u is not None
    l2 = (all(v["rmse_law"] <= 2 * REF for v in uns.values()) and pooled_u <= 1.5 * REF) if ran else None
    out["bars"]["L2_unseen"] = {"per_world": {n: v["rmse_law"] for n, v in uns.items()}, "pooled": pooled_u, "limits": [2 * REF, 1.5 * REF], "holds": l2}
    if ran and pooled_u > 3 * REF:
        fail = True
    # L3 real
    ...
    # L5 against nominal dimension
    rs = rows_of({"unseen", "real", "scale"})
    if rs:
        el, ec = rmse(law, rs), rmse(comp, rs); l5 = el <= 0.8 * ec
        if ec < el:
            fail = True
    else:
        el = ec = None; l5 = None
    out["bars"]["L5_vs_nominal"] = {"rmse_law": el, "rmse_competitor": ec, "holds": l5}
    # X1 latent rank
    ...
    if sum(v > FX for v in xr.values()) > len(xr) / 2:
        fail = True
    # P1 polarity
    pr = {w["name"]: {"hub_jaccard_same": w["polarity"]["hub_jaccard_same_alpha_mean"], "hub_jaccard_diff": w["polarity"]["hub_jaccard_diff_alpha_mean"],
                      "reversal_same": w["polarity"]["reversal_frac_same_alpha_mean"], "reversal_diff": w["polarity"]["reversal_frac_diff_alpha_mean"]}
          for w in res["worlds"] if w["polarity"]["pairs_same"] > 0}
    ...
    out["bars"]["B1_budget_reported"] = {k: {"changed_by_budget": v, "nondecreasing": all(v[a] <= v[b] + 1e-12 for a, b in zip(sorted(v, key=float)[:-1], sorted(v, key=float)[1:])), "small_budget_quiet": v.get("0.1", 1.0) <= 0.05} for k, v in b1.items()}
