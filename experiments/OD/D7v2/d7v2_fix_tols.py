"""Fixes the D7v2 tolerances from the pilot by the rules of PREREG-D7V2 Section 5 and prints the pilot summary.
...
from d7v2_grade import cells, adv, R_MIN  # noqa: E402
...
tols = {"M2": M2, "TOL2": TOL2, "TOL_N": TOL_N, "R_MIN": R_MIN, "pilot": {"pooled_adv_readenergy_in_scope": float(np.mean(adv2)), "min_adv_readenergy_in_scope": float(min(adv2)), "pooled_by_n": pa, "pooled_adv_below_scope": float(np.mean(advb)) if advb else None},
        "rule": "PREREG-D7V2 Section 5: scope ranks >= 64; M2 = 0.5 x pilot in-scope pooled read-energy advantage rounded down to 0.01 (at least 0); TOL2 = 1.25 x pilot max in-scope read-energy shortfall rounded up to 0.01; TOL_N = 1.5 x the change of the in-scope pooled advantage between the pilot resolutions rounded up to 0.01"}
