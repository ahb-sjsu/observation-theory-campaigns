"""D6: sensor placement as the choice of an observer under a budget (OD track, gate D6).
...
directions (eigenvectors of W_S with lambda <= B^2 / rho^2), of size rho, grows to a tolerance.
    ...
    python d6_placement.py --selftest
...
def run_world(world: dict, cfg: dict, seed: int, log=print) -> dict:
    ...
    for frac in cfg["threshold_fractions"]:
        ...
        for m in cfg["required_counts"]:
            ...
            S = greedy(A, C, T, c, m, kmax); k = len(S); Wg = gramian(A, C[S], T)
            cell.update({"greedy": S, "k": k, "d_obs_greedy": d_obs(Wg, c)})
...
def selftest() -> int:
    ...
    # greedy reaches m when feasible and never exceeds the exhaustive minimum by more than the candidates allow
    A = world_matrix("lorenz96", 8, {"F": 8.0}); C = candidate_sensors("lorenz96", 8, rng, {}); Wa = gramian(A, C, 0.5); c = 1e-3 * np.linalg.eigvalsh(Wa).max()
    S = greedy(A, C, 0.5, c, 4, 8); ks = exhaustive_min_count(A, C, 0.5, c, 4, len(S), 100000)
    ok = d_obs(gramian(A, C[S], 0.5), c) >= 4 and ks is not None and ks <= len(S); print("greedy reaches m = 4 with", len(S), "sensors; exhaustive minimum", ks, ok); fails += 0 if ok else 1
