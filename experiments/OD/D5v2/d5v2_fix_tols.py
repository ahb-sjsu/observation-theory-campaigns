"""Fixes the D5 tolerances from the pilot by the rules of PREREG-D5V2 Section 5 and prints the pilot summary.
TOL_N = 1.5 x the pilot's largest resolution difference (N against 2N, same initial condition and viscosity),
...
from d5v2_grade import control_points, NU_MAX  # noqa: E402
...
print("resolution differences:", {k: round(v, 4) for k, v in diffs.items()})
...
tols = {"TOL_N": TOL_N, "TOL_C": TOL_C, "RATIO": RATIO, "pilot": {"resolution_diffs": diffs, "collapse": coll},
        "rule": "PREREG-D5V2 Section 5: TOL_N = 1.5 x pilot max in-scope resolution difference rounded up to 0.01; TOL_C = 1.5 x pilot max in-scope per-viscosity median bin IQR of the scaled collapse rounded up to 0.01; RATIO = 1.25 x the pilot's largest in-scope scaled-over-unscaled ratio rounded up to 0.05, at most 0.9"}
