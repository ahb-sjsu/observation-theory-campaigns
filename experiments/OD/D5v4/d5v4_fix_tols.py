"""Fixes the D5 tolerances from the pilot by the rules of PREREG-D5V4 Section 5 and prints the pilot summary.
...
from d5v4_grade import control_points, NU_MAX  # noqa: E402
...
from d5v4_grade import grade as _grade_v3  # noqa: E402
raw_diffs = diffs
...
for w in ctrl:
    scc = scatter(control_points(w, True)); unc = scatter(control_points(w, False))
    print("control %s: scaled median IQR %s, unscaled %s (per control)" % (w["name"], scc["median_iqr"], unc["median_iqr"]))
...
tols = {"TOL_N": TOL_N, "TOL_C": TOL_C, "RATIO": RATIO, "pilot": {"resolution_diffs_normalised_B8plus": diffs, "resolution_diffs_raw_all_budgets": raw_diffs, "collapse": coll},
        "rule": "PREREG-D5V4 Section 5: TOL_N = 1.5 x pilot max in-scope N1 difference on f_B(t)/f_B(0) at budgets 8 and above rounded up to 0.01; TOL_C = 1.5 x pilot max in-scope per-viscosity median bin IQR of the scaled collapse rounded up to 0.01; RATIO = 1.25 x the pilot's largest in-scope scaled-over-unscaled ratio rounded up to 0.05, at most 0.9"}
