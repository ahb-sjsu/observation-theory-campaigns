"""Grades a D5v4 results.json against the sealed bars of PREREG-D5V4.md. D5v3's grader with the control bar C3 taken
PER CONTROL (each control's scaled scatter against its own unscaled scatter), the revision D5v3's record named: pooled, the
scaled variable separates controls with different front wavenumbers and reads as a collapse. D5v3 = D5v2's grader with the convergence bar N1 taken
on f_B(t) / f_B(0) over budgets B_MIN_N = 8 and above (the quantity the collapse uses, at the budgets the coarser grid
resolves), the revision D5v2's record named.
Usage: python d5v4_grade.py results.json --tols tolerances.json [--out grade.json]
...
def grade(res: dict, tols: dict) -> dict:
    ...
    out = {"tolerances": tols, "scope_nu_max": NU_MAX, "convergence_budget_min": B_MIN_N, "bars": {}, "gate": None}; fail = False
    ...
    for a in ins:
        for b in ins:
            if a["ic_seed"] == b["ic_seed"] and a["nu"] == b["nu"] and b["N"] == 2 * a["N"]:
                ...
                shared = [bb for bb in a["record"]["geom"][0] if bb != "total_energy" and bb in b["record"]["geom"][0] and int(bb) >= B_MIN_N]

                def _ratio(w, bb):
                    f = np.array([gg[bb]["read_fraction"] for gg in w["record"]["geom"][:n]]); return f / f[0]
                diffs[f"{a['name']}|{b['name']}"] = max(float(np.max(np.abs(_ratio(a, bb)[keep] - _ratio(b, bb)[keep]))) for bb in shared)
    ...
    if len(ctrl) >= 1:
        per = {}
        for w in ctrl:
            sc = scatter(control_points(w, True)); un = scatter(control_points(w, False))
            ok = (sc["median_iqr"] is not None and un["median_iqr"] is not None and sc["median_iqr"] >= RATIO * un["median_iqr"])
            per[w["name"]] = {"scaled": sc, "unscaled": un, "ratio": (sc["median_iqr"] / un["median_iqr"]) if (sc["median_iqr"] and un["median_iqr"]) else None, "holds": bool(ok)}
            if sc["median_iqr"] is not None and un["median_iqr"] is not None and sc["median_iqr"] < 0.5 * un["median_iqr"]: fail = True
        c3 = all(v["holds"] for v in per.values())
        out["bars"]["C3_control_does_not_collapse"] = {"per_control": per, "holds": bool(c3)}
    else:
