"""Fixes the D2 tolerances from the pilot by the rule of PREREG-D2V3 Section 5 and prints the pilot
...
for w in res["worlds"]:
    ...
    print("%-18s %-8s rmse law %.3f nominal %.3f d2 %.3f d2v2 %.3f | jac same %.3f diff %.3f | skew range %.2f..%.2f" % (
        w["name"], w["group"], rmse(law, rs), rmse(comp, rs), rmse(d2, rs), rmse(d2v2, rs), p["hub_jaccard_same_alpha_mean"], p["hub_jaccard_diff_alpha_mean"],
