"""Grades a D2v5 results.json against the sealed bars of PREREG-D2V5.md.
Usage: python d2v5_grade.py results.json --tols tolerances.json [--out grade.json]
...
def grade(res: dict, tols: dict) -> dict:
    law = res["law"]["law"]; comp = res["law"]["competitor_nominal"]; d2 = res["law"]["competitor_d2"]; d2v2 = res["law"]["competitor_d2v2"]; d2v3 = res["law"]["competitor_d2v3"]; d2v4 = res["law"]["competitor_d2v4"]
    REF = float(tols["REF"]); FX = float(tols["FRAC_X"])
    out = {"tolerances": tols, "law": law, "competitor_nominal": comp, "competitor_d2": d2, "competitor_d2v2": d2v2, "competitor_d2v3": d2v3, "competitor_d2v4": d2v4, "groups": {}, "bars": {}, "gate": None}
    fail = False
    ...
    per_world = {w["name"]: {"group": w["group"], "rmse_law": rmse(law, law_rows(w)), "rmse_nominal": rmse(comp, law_rows(w)), "rmse_d2": rmse(d2, law_rows(w)), "rmse_d2v2": rmse(d2v2, law_rows(w)), "rmse_d2v3": rmse(d2v3, law_rows(w)), "rmse_d2v4": rmse(d2v4, law_rows(w))} for w in res["worlds"]}
    out["worlds"] = per_world
    for g in ("train", "heldout", "unseen", "real", "scale"):
        rs = rows_of({g})
        out["groups"][g] = {"n": len(rs), "rmse_law": rmse(law, rs) if rs else None, "rmse_nominal": rmse(comp, rs) if rs else None, "rmse_d2": rmse(d2, rs) if rs else None, "rmse_d2v2": rmse(d2v2, rs) if rs else None, "rmse_d2v3": rmse(d2v3, rs) if rs else None, "rmse_d2v4": rmse(d2v4, rs) if rs else None}
    # L1 fresh seeds
    ...
    if rs:
        el, en, ed, ev, e3, e4 = rmse(law, rs), rmse(comp, rs), rmse(d2, rs), rmse(d2v2, rs), rmse(d2v3, rs), rmse(d2v4, rs)
