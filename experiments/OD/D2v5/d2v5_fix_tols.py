"""Fixes the D2 tolerances from the pilot by the rule of PREREG-D2V5 Section 5 and prints the pilot
...
from d2v5_hubness import rmse  # noqa: E402
...
for w in res["worlds"]:
    ...
    print("%-18s %-8s rmse law %.3f nominal %.3f d2 %.3f d2v2 %.3f d2v3 %.3f d2v4 %.3f | jac same %.3f diff %.3f | skew range %.2f..%.2f" % (
        w["name"], w["group"], rmse(law, rs), rmse(comp, rs), rmse(d2, rs), rmse(d2v2, rs), rmse(d2v3, rs), rmse(d2v4, rs), p["hub_jaccard_same_alpha_mean"], p["hub_jaccard_diff_alpha_mean"],
...
tols = {"REF": float(pooled), "REF_note": "the frozen law's RMSE pooled over every pilot row (training and held-out), the held-out RMSE alone being " + "%.4f" % law["heldout_rmse"], "pooled_nominal": rmse(comp, all_rows), "pooled_d2_law": rmse(d2, all_rows), "pooled_d2v2_law": rmse(d2v2, all_rows), "pooled_d2v3_law": rmse(d2v3, all_rows), "pooled_d2v4_law": rmse(d2v4, all_rows), "FRAC_X": min(math.ceil(max(ratios.values()) * 1.5 * 100) / 100, 0.9),
        "rule": "PREREG-D2V5 Section 5: REF = frozen law's held-out RMSE at the pilot; FRAC_X = 1.5 x pilot max rank ratio rounded up to 2 decimals, at most 0.9",
