"""Grades a D6v3 results.json against the sealed bars of PREREG-D6V3v2.md.
...
def grade(res: dict, tols: dict) -> dict:
    ...
    unreachable = [c for c in cells if c["feasible"] and c.get("feasible_at_cap") is False]
    feas = [c for c in cells if c["feasible"] and c.get("feasible_at_cap") is not False]
    out["n_cells"] = len(cells); out["n_feasible"] = len(feas); out["n_unreachable_at_cap"] = len(unreachable)
    out["unreachable_at_cap"] = [(c["world"], c["frac"], c["m"], c["d_obs_all"], c["d_obs_greedy"]) for c in unreachable]
