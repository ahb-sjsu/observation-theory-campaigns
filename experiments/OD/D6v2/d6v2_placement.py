...
def exact_subset(A: np.ndarray, C: np.ndarray, T: float, c: float, m: int, kmax: int, cap: int) -> list[int] | None:
    """The smallest sensor set reaching the count, by exhaustive search in index order, where every subset
    size up to the answer has at most cap subsets (the same cap as the exhaustive reference); None otherwise."""
    n = C.shape[0]
    for k in range(1, kmax + 1):
        if math.comb(n, k) > cap: return None
        for S in itertools.combinations(range(n), k):
            if d_obs(gramian(A, C[list(S)], T), c) >= m: return [int(i) for i in S]
    return None
...
def lookahead(A: np.ndarray, C: np.ndarray, T: float, c: float, m: int, kmax: int, cap: int = 200000) -> list[int]:
    ...
    E = exact_subset(A, C, T, c, m, kmax, cap)
...
def selftest() -> int:
    ...
    S = greedy(A, C, 0.5, c, 4, 8); L = lookahead(A, C, 0.5, c, 4, 8); ks = exhaustive_min_count(A, C, 0.5, c, 4, len(S), 100000)
