"""Fixes the D2 tolerances from the pilot by the rule of PREREG-D2V6 Section 5 and prints the pilot
...
d2 = res["law"]["competitor_d2"]; print("D2 frozen law: train rmse %.4f heldout rmse %.4f" % (res["law"]["d2_law_train_rmse"], res["law"]["d2_law_heldout_rmse"]))
d2v2 = res["law"]["competitor_d2v2"]; print("D2v2 frozen law: train rmse %.4f heldout rmse %.4f" % (res["law"]["d2v2_law_train_rmse"], res["law"]["d2v2_law_heldout_rmse"]))
d2v3 = res["law"]["competitor_d2v3"]; print("D2v3 frozen law: train rmse %.4f heldout rmse %.4f" % (res["law"]["d2v3_law_train_rmse"], res["law"]["d2v3_law_heldout_rmse"]))
...
print("law transform:", law.get("transform"))
...
for w in res["worlds"]:
    ...
    print("%-18s %-8s rmse law %.3f nominal %.3f d2 %.3f d2v2 %.3f d2v3 %.3f d2v4 %.3f d2v5 %.3f | jac same %.3f diff %.3f | skew range %.2f..%.2f" % (
        w["name"], w["group"], rmse(law, rs), rmse(comp, rs), rmse(d2, rs), rmse(d2v2, rs), rmse(d2v3, rs), rmse(d2v4, rs), rmse(d2v5, rs), p["hub_jaccard_same_alpha_mean"], p["hub_jaccard_diff_alpha_mean"],
        min(r["targets"]["skew"] for r in rs), max(r["targets"]["skew"] for r in rs)))
    for r in w["rows"]:
        if "budget_rel" in r:
...
tols = {"REF": float(pooled), "REF_note": "the frozen law's RMSE pooled over every pilot row (training and held-out), the held-out RMSE alone being " + "%.4f" % law["heldout_rmse"], "pooled_nominal": rmse(comp, all_rows), "pooled_d2_law": rmse(d2, all_rows), "pooled_d2v2_law": rmse(d2v2, all_rows), "pooled_d2v3_law": rmse(d2v3, all_rows), "pooled_d2v4_law": rmse(d2v4, all_rows), "pooled_d2v5_law": rmse(d2v5, all_rows), "FRAC_X": min(math.ceil(max(ratios.values()) * 1.5 * 100) / 100, 0.9),
        "rule": "PREREG-D2V6 Section 5: REF = frozen law's held-out RMSE at the pilot; FRAC_X = 1.5 x pilot max rank ratio rounded up to 2 decimals, at most 0.9",
