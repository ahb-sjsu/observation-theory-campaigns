"""Grades a D2v2 results.json against the sealed bars of PREREG-D2V2.md.
...
from d2v2_hubness import rmse  # noqa: E402


def grade(res: dict, tols: dict) -> dict:
    law = res["law"]["law"]; comp = res["law"]["competitor_nominal"]; d2 = res["law"]["competitor_d2"]
    REF = float(tols["REF"]); FX = float(tols["FRAC_X"])
    out = {"tolerances": tols, "law": law, "competitor_nominal": comp, "competitor_d2": d2, "groups": {}, "bars": {}, "gate": None}
    fail = False
    ...
    per_world = {w["name"]: {"group": w["group"], "rmse_law": rmse(law, law_rows(w)), "rmse_nominal": rmse(comp, law_rows(w)), "rmse_d2": rmse(d2, law_rows(w))} for w in res["worlds"]}
    out["worlds"] = per_world
    for g in ("train", "heldout", "unseen", "real", "scale"):
        rs = rows_of({g})
        out["groups"][g] = {"n": len(rs), "rmse_law": rmse(law, rs) if rs else None, "rmse_nominal": rmse(comp, rs) if rs else None, "rmse_d2": rmse(d2, rs) if rs else None}
    # L1 fresh seeds
    ...
    if rs:
        el, en, ed = rmse(law, rs), rmse(comp, rs), rmse(d2, rs)
        per_group = {g: (out["groups"][g]["rmse_law"], out["groups"][g]["rmse_nominal"], out["groups"][g]["rmse_d2"]) for g in ("unseen", "real", "scale") if out["groups"][g]["n"]}
        l5 = el <= 0.8 * en and el <= 0.8 * ed and all(a <= b and a <= c for a, b, c in per_group.values())
        if en < el or ed < el:
            fail = True
    else:
        el = en = ed = None; per_group = {}; l5 = None
    out["bars"]["L5_vs_competitors"] = {"rmse_law": el, "rmse_nominal": en, "rmse_d2": ed, "per_group_law_nominal_d2": per_group, "holds": l5}
    # X1 latent rank
