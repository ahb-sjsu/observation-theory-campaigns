"""D2v8: observer-relative hubness, the excess law with full-dimensional worlds at d = 384 to 512
...
real corpora and the scale cells; it never refits.
    ...
    python d2v8_hubness.py --selftest
...
for _v in BASE:
    ...
    if _v not in ("kurt_r", "skew_r", "kurt_1d", "kurt_lrk"):
...
D2V5_FROZEN_LAW = {"target": "skew", "features": ["inv_cv_d", "log_id_twonn", "sqrt_d_eff"], "coef": [-2.8022873414756475, -0.5096103778790827, 1.1904884065877568, 1.066977067416814], "transform": "identity",
...
D2V4_FROZEN_LAW = {"target": "skew", "features": ["cv_r_x_sqrt_d_eff", "log_d_nom_x_log_k", "log_id_twonn"], "coef": [-1.0937946857408793, 0.16852132160750913, -0.03188415704240788, 0.7013760762316192], "transform": "log1p",
...
D2V3_FROZEN_LAW = {"target": "skew", "features": ["inv_cv_knn", "log_id_twonn", "sqrt_d_eff"], "coef": [-1.462907521326016, -0.013182421393936017, 0.7474954718371633, 0.06855322431693047], "transform": "log1p",
...
D2V7_FROZEN_LAW = {"target": "log_excess", "features": ["inv_cv_knn", "log_d_eff", "sqrt_id_twonn"], "coef": [-1.1031826827055975, -0.025315490538022678, 0.16656642745723538, 0.4337968152191875], "transform": "identity",
...
def run(cfg: dict, seed_role: str, out_path: str, discover_path: str | None, law_path: str | None) -> dict:
    ...
    if law_path:
        ...
        for w in result["worlds"]:
            ...
            result["evaluation"][w["name"]] = {"group": w["group"], "rmse_law": rmse(frozen["law"], rs), "rmse_competitor": rmse(frozen["competitor_nominal"], rs), "rmse_unity": rmse(frozen["competitor_unity"], rs), "rmse_d2v7_law": rmse(frozen["competitor_d2v7"], rs), "n": len(rs)}
...
def selftest() -> int:
    ...
    law2 = discover(rows2, rows2, "skew", 1, {"log_d_eff", "d_eff"}, transforms=("identity", "log1p")); ok = law2["transform"] == "log1p" and law2["features"] == ["log_d_eff"] and abs(law2["coef"][1] - 0.5) < 1e-6
    ...
    row = {"variables": {"cv_knn": 0.167, "id_twonn": 11.8, "d_eff": 32.4}, "targets": {"skew": 0.24}}
    ...
    # tail variables: a ball, a Gaussian and Student t with 3 degrees order by tail weight in every neighbour-distance
    ...
    row = {"variables": {"cv_d": 0.2, "id_twonn": 20.0, "d_eff": 40.0}, "targets": {"skew": 1.0}}
    ...
    # hub excess: counts with one point at 3 x the Poisson ceiling read excess 3 and log excess log 3; the unity law predicts 0
    ...
    row = {"variables": {"cv_knn": 0.2, "d_eff": 40.0, "id_twonn": 20.0}, "targets": {"log_excess": 1.0}}
