"""Grades a D2v8 results.json against the sealed bars of PREREG-D2V8.md.
Usage: python d2v8_grade.py results.json --tols tolerances.json [--out grade.json]
...
def grade(res: dict, tols: dict) -> dict:
    law = res["law"]["law"]; comp = res["law"]["competitor_nominal"]; unity = res["law"]["competitor_unity"]; d2v7 = res["law"]["competitor_d2v7"]
    REF = float(tols["REF"]); FX = float(tols["FRAC_X"])
    out = {"tolerances": tols, "law": law, "competitor_nominal": comp, "competitor_unity": unity, "competitor_d2v7": d2v7, "groups": {}, "bars": {}, "gate": None}
    fail = False
    ...
    per_world = {w["name"]: {"group": w["group"], "rmse_law": rmse(law, law_rows(w)), "rmse_nominal": rmse(comp, law_rows(w)), "rmse_unity": rmse(unity, law_rows(w)), "rmse_d2v7": rmse(d2v7, law_rows(w))} for w in res["worlds"]}
    out["worlds"] = per_world
    for g in ("train", "heldout", "unseen", "real", "scale"):
        rs = rows_of({g})
        out["groups"][g] = {"n": len(rs), "rmse_law": rmse(law, rs) if rs else None, "rmse_nominal": rmse(comp, rs) if rs else None, "rmse_unity": rmse(unity, rs) if rs else None, "rmse_d2v7": rmse(d2v7, rs) if rs else None}
    ...
    if rs:
        el, en, eu, e7 = rmse(law, rs), rmse(comp, rs), rmse(unity, rs), rmse(d2v7, rs)
