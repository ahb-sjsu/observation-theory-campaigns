"""Fixes the D2v7 tolerances from the pilot by the rule of PREREG-D2V8 Section 5 and prints the pilot
...
from d2v8_hubness import rmse  # noqa: E402
...
for w in res["worlds"]:
    ...
    print("%-18s %-8s rmse law %.3f nominal %.3f unity %.3f d2v7 %.3f | jac same %.3f diff %.3f | excess range %.2f..%.2f" % (
        w["name"], w["group"], rmse(law, rs), rmse(comp, rs), rmse(unity, rs), rmse(d2v7, rs), p["hub_jaccard_same_alpha_mean"], p["hub_jaccard_diff_alpha_mean"],
...
tols = {"REF": float(pooled), "REF_note": "the frozen law's RMS error in log excess pooled over every pilot row (training and held-out), the held-out error alone being " + "%.4f" % law["heldout_rmse"], "pooled_nominal": rmse(comp, all_rows), "pooled_unity": rmse(unity, all_rows), "pooled_d2v7_law": rmse(d2v7, all_rows), "FRAC_X": min(math.ceil(max(ratios.values()) * 1.5 * 100) / 100, 0.9),
        "rule": "PREREG-D2V8 Section 5: REF = the frozen law's RMS log-ratio error pooled over every pilot row; FRAC_X = 1.5 x pilot max rank ratio rounded up to 2 decimals, at most 0.9",
