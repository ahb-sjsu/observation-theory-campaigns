...
since the last sync to the band a service shares along its path, which yields a
freshness budget of about eight such adds at a $5\%$ risk target. Which services are exposed when the twin is
...
grid of 76. A certificate selects the highest of five declared formats whose
...
to time. A band taking one add per week can hold a sync for about two months. A
busy one taking one add per day must sync roughly weekly.
...
practice of one uniform system margin, with the deployed fill taken from real
traffic. Routing the 1482 demands of the SNDlib janos-us-ca matrix~\cite{sndlib}
(39 nodes, 61 undirected links) gives a per-link channel-fill distribution with
median 10 and 90th percentile 35 of 76; each CORONET lightpath's deployed band
fill is sampled from that distribution. Real traffic is heavy-tailed, so a uniform
margin sized for the few near-full links over-provisions the lightly filled
majority. Giving both
...
fleet-mean budget of $\bar{m}=1.0$\,dB. Policy A allocates by reach,
$m_i^{A}=\bar{m}\,r_i/\bar{r}$ with $r_i$ the service's span count normalised to
the longest route. Policy B allocates the same total by band-centrality,
$m_i^{B}=\bar{m}\,c_i/\bar{c}$ with $c_i=1-2\,|p_i-\tfrac12|+0.05$ for spectral
position $p_i\in[0,1]$. Long-reach band-edge services do better under A
...
the average. The evidence stands on the GN model with real demand data, and its
...
GN-model GSNR rather than a receiver's pre-FEC BER. Each of these moves deployed
GSNR further from the twin's synced view, so they tighten the budget rather than
relax it. A coherent-receiver witness on a testbed twin is the natural next step.
