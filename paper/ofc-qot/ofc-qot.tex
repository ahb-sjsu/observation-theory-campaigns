...
capacity over a uniform margin. Two margin policies with equal budgets can receive
opposite verdicts from two service classes, so a fleet-level number can serve
neither. And when the twin's QoT model is a learned surrogate, training it on
average error leaves it blind exactly at the FEC threshold where its decisions are
made.
...
control makes the mechanism sharp. Running the same protocol on two FEC classes,
whose thresholds differ by a shift but whose reads are the same projection of the
network state, produces no inversion at all. The inversion needs consumers that read
different axes of the state, here reach against spectral position, and that is
exactly what mixed service portfolios do. A twin that reports one fleet-level number
answers a question no consumer asked.
