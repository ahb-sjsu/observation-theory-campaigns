...
\twocolumn[
  ...
  reproduces the identical content and figures. Numbers are sealed and graded or
  deterministic, except the verdict-inversion figures marked for the 2026-08-27 seal.\par}
  \medskip

  \noindent\textbf{Abstract:} A GNPy-class digital twin makes format decisions between
  telemetry syncs while the network fills. We measure when its decisions fail:
  false-clears grow linearly with un-synced channel adds, giving an eight-add
  freshness budget.
...
exposure map, the verdict inversion, and the price of the wrong training objective.
...
threshold-adjacent services first, report per-consumer risk rather than one fleet
number, and train surrogates on the decision rather than the average. The evidence
stands on the GN model with real demand data. A coherent-receiver pre-FEC-BER
witness on a testbed twin is the natural next step. The same consumer-relative
failure appears in radio link adaptation and in database
