...
\cite{Whitney1955, ArnoldGZ}.  On the monotone null the instrument
reports one
...
\cite{Stueckelberg1941, HorwitzPiron1973, SaadHorwitzArshansky1989}.
Land's impulsive Coulomb scattering configuration was transcribed from
the preprint equation by equation with its
...
$0.65$ to $0.9115$ at field $0.80$, three and a half decades across a
step of $0.15$ in the field, and only two of the eighteen cells landed
in the measurable band at all.  Fixed rectangular grids therefore buy
...
\nextgroupplot[ybar, bar width=13pt, ylabel={oscillation statistic},
  xtick={1,2,3,4},
  xticklabels={null mean, null 95th, classical residual, quantum},
  x tick label style={rotate=28, anchor=east, font=\footnotesize},
  xmin=0.4, xmax=4.6, ymin=0, ymax=1.18,
...
same binomial counts, with the declared smooth overlap model removed
from the curve and from every null curve alike, so all four bars are
measured after the same treatment.  The classical residual sits below
that null's mean at a p-value of 0.757 and the quantum value sits past
every null draw.  The classical value before the removal is 0.4480 and
belongs with the null before the removal, so it is reported in the text
and not plotted beside these.  All values are read from committed
evidence records.}
\label{fig:pf7}
\end{figure}
...
tail knows where a function's poles are, and the adiabatic exponent of
a two-level crossing is set by the nearest complex degeneracy of its
adiabatic levels, which for an analytic force profile is fixed by that
profile's nearest singularity
\cite{Dykhne1962, DavisPechukas1976}.  Writing $d$ for the
...
precision they appeared to have.  It passes all eight of its items.
The four fitted
...
of $5\times10^{-4}$, $2\times10^{-4}$, and $10^{-4}$, with the
extended
...
\emph{Singularities of Differentiable Maps, Volume 1. The
Classification of Critical Points, Caustics and Wave Fronts}
(Birkh\"auser, Boston, 1985).

\bibitem{Stueckelberg1941}
E.~C.~G.~Stueckelberg,
Remarque \`a propos de la cr\'eation de paires de particules en
...
D.~Saad, L.~P.~Horwitz, and R.~I.~Arshansky,
Off-shell electromagnetism in manifestly covariant relativistic quantum
mechanics,
Found. Phys. \textbf{19}, 1125 (1989).

\bibitem{Land2016}
M.~Land,
Pair production in classical Stueckelberg-Horwitz-Piron
electrodynamics,
arXiv:1604.01625 (2016),
published as J. Phys.: Conf. Ser. \textbf{615}, 012007 (2015).
