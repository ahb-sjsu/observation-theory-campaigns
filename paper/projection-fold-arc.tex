...
instrument net reproduces the exact controls and refuses the traps, a
...
belonging to the declaration rather than to the model.
...
\section{The instrument net and the negative control}
\label{sec:net}
...
which is what singularity theory predicts and what makes the rest of the
arc worth running.

The negative control is the campaign's most useful early measurement.
A bounded local Hamiltonian with an oscillator in the observed time, an
...
That uniformity is the point.  A rate is a property of the
...
$5 \times 10^4$ members each, with relative drift uniformly near
$6 \times 10^{-7}$, the family has a critical field above which the
deterministic crossing kick alone reverses the event, and below it
reversal is fluctuation driven.  The transition is razor sharp.  At gap
...
\begin{axis}[paperaxis, height=0.62\columnwidth,
  ...
  ylabel={$-\log f$},
  xmin=0.04, xmax=0.235, ymin=-0.5, ymax=9.6,
...
bisection, which is how the manifest landed outside the regime where
reversals occur.
...
forty natural units does not reach the asymptotic regime at that gap.
...
\begin{axis}[paperaxis, ybar, bar width=9pt, height=0.56\columnwidth,
  ...
  ylabel={observations},
  xtick={0,1,2,3}, xmin=-0.6, xmax=3.6, ymin=0, ymax=470,
...
applied to every width, so the three widths were compared in different
dynamical regimes and the narrowest cells sat partly outside the
adiabatic regime where the exponential form is asymptotic.
...
naive value and the sech-squared at $0.524$ of it.  The run's verdict
...
observable's definition before anything about it was sealed, which is
what a gate exists to do.  The conserved quantity of the extended
...
of $10^{-10}$.  The diagnosis is that the arctangent tilt approaches
...
$3 \times 10^{-7}$ against the same $10^{-10}$, which is a bar set
...
steadily, while the sech-squared transfer is unchanged to the digits
recorded, so the effect belongs to algebraic tails and not to the
method.  The exponent is far more robust than the transfer, changing by
at most $0.28$ percent between the nearest and furthest spans, and the
...
their mean against a declared bar of two percent, which is the sealed
...
\section{Five vacuous runs and four bars that failed for the
declaration}
\label{sec:spine}

This arc contains the densest failure record in the campaign, and the
record is load-bearing rather than decorative, so it is reported here
as a sequence of facts with each design error named.
...
widths in different dynamical regimes, so its cross-profile ratio test
was contaminated and could not be read.
...
$10^{-10}$ that the declared timestep could not deliver, missing at
...
None of this is offered as evidence of rigor.  It is offered because
the sequence of verdicts is part of the evidence and because five of
these failures would have been reported as successes by a pipeline that
did not check whether its manifest contained its events.
...
uniformly near $6\times10^{-7}$, a razor-sharp transition from
...
in different dynamical regimes & measured in model, exploratory \\
...
1.291 percent against a bar of $10^{-10}$ because the tilt approaches
...
while the sech-squared transfer is unchanged to the digits recorded,
the exponent changing by at most 0.28 percent between the nearest and
furthest spans, and width universality surviving at the furthest span
with a spread of 0.36 percent & measured in model, exploratory \\
...
about a declared finite model, and the paper's scope row says so once
more.
...
field probes, the decay gate with its blind probe, the eight records of
