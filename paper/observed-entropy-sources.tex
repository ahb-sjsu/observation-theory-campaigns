...
The scope of these claims is narrow and stated once.  Every result is a
computation or a measured simulation in a declared toy model class, graded
by the campaign's evidence scheme as demonstrated in model.  No claim is
...
The track now comprises six experiments.  The fold theorem and the exact
...
is ordinary linear dispersion and is common to both.  An honest reading
must therefore refuse the total rise as a discriminator.  The late-time
...
a mean unsigned crossing multiplicity between $9.6$ and $10.2$ per
...
with no estimator and no sampling error.
...
plugin mutual information between the binned observed coordinate and the
binned bath energy.
...
observer can flip, and the published claim that observed entropy carries
...
between $0.009$ and $0.016$ bits, within a factor of two of its
...
gaps pricing orientation and branch knowledge separately & demonstrated
in model \\
...
invisible to coarse bath observables & demonstrated in model \\
\end{tabular}
\end{ruledtabular}
\end{table*}
