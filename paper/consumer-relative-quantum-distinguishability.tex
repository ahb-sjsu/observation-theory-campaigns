...
\date{August 4, 2026 (v2, with sealed preregistered confirmations)}
...
one shared event.  The second and third findings were subsequently
confirmed under sealed preregistrations executed on scenario and coupling
grids fully disjoint from the exploratory runs, with margins and
thresholds fixed in advance.  We also report three instrument defects found during
...
existence in a declared model class and nothing stronger.  The
exploratory runs above were then subjected to a sealed preregistered
replication, registration PREREG-QO2-001, whose scenario grids are fully
disjoint from every angle used above and whose win margins of $0.01$ on
both axes were fixed before execution.  The governed run confirms the
flip at the scarce budget at the primary coupling point, with task
distortions $0.0000$ against $0.1076$ and infidelities $0.3637$ against
$0.0678$, and at both preregistered secondary coupling points, with the
anti-arm worst on the task everywhere.  Within this model class and
encoder family the flip therefore carries the demonstrated-in-model
label; any claim beyond this class would require new preregistration.
...
point survives every requirement, so a selection-effect claim requires a
preregistered spread threshold.  And in one dimension every cut has a
boundary of a single site, so this experiment tests the universality
content of the quantifier and says nothing about area scaling.

The preregistered study exists.  Registration PREREG-QO3-001 fixed the
spread threshold at $0.05$ before execution and moved to a coupling grid
and excitation grid fully disjoint from those above, stating activation
and inertness as separately refutable claims.  The governed run passes
both.  The joint shared-event requirement holds on one quarter of the
fresh grid against single-window survivals of one, the disjoint-probe
requirement adds exactly nothing beyond its tightest single consumer,
and the surviving set is exactly the weak-coupling row of the fresh
grid, reproducing the predicted secondary structure.  Within this model
family both claims therefore carry the demonstrated-in-model label.
...
finite-dimensional models.  The flip and the consumer-family results
were confirmed under sealed preregistrations on grids disjoint from
their exploratory runs and carry the demonstrated-in-model label for
their declared model classes; the remaining results are exploratory.
Nothing in the paper claims anything about physical gravity, real
horizons, or quantum gravity.  The consumer does not create anything
physical by choosing what to observe.
