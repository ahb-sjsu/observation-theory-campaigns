...
consumer promoted $3$ of them to candidates (Fig.~\ref{fig:cadence}). The false-clear rate of the
raw single-scan detector, relative to the cadence consumer, is therefore
$1 - 3/6651 \approx 0.9995$. A signature on any of these frames would have certified all $6651$ as
authentic and unaltered, and would have separated none of them. The discrimination comes from
re-observation, not from provenance.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{build/cadence_fig.pdf}
  \caption{The re-observation consumer on real data. (a) Raw narrowband hits in each scan of the
  six-scan ON-OFF cadence of HIP101027 (GBT L-band, MJD 57574, coarse channels 119--144). (b) Of
  $6651$ raw on-source hits, the cadence keeps $3$. One measurement, and $99.95\%$ of the
  ``detections'' a signature would certify are gone.}
  \label{fig:cadence}
\end{figure}
% TODO: optionally add a parameters table (target, MJD, msm IDs, snr/drift settings).
